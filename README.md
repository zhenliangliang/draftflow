...
如需启用 AI 内容分析，进入「内容雷达」，点击「配置 AI」，选择 OpenAI 或 LiteLLM 网关，填写接口地址和 API Key 后获取可用模型列表。保存前会验证模型权限，密钥加密写入数据库；LiteLLM 地址可以填写网关根地址或带 `/v1` 的地址。未配置时仍可使用基础热度评分、规则选题和提纲生成功能。
